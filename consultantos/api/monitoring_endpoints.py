...
@router.put("/{monitor_id}/notification-preferences")
async def update_notification_preferences(
    ...
    """
    Update notification preferences for a monitor.

    Configure which channels to use and channel-specific settings
    (email addresses, Slack webhooks, etc.).
    ...
    Returns:
        Updated monitor instance

    Raises:
        HTTPException: 404 if monitor not found
        HTTPException: 403 if user doesn't own monitor
    ...
    try:
        ...
        if request.notification_channels is not None:
            monitor.config.notification_channels = request.notification_channels

        if request.notification_preferences is not None:
            monitor.config.notification_preferences = request.notification_preferences

        # Save updated monitor
        await monitor_service.db.update_monitor(monitor_id, user_id, monitor.dict())

        logger.info(
            "notification_preferences_updated",
            monitor_id=monitor_id,
            user_id=user_id,
            channels=[c.value for c in monitor.config.notification_channels]
        )
...
@router.get("/{monitor_id}/notification-history")
async def get_notification_history(
    ...
    """
    Get notification delivery history for a monitor.

    Shows which alerts were sent, to which channels, and delivery status.
    ...
    try:
        ...
        # Get delivery records from Firestore
        deliveries_collection = f"monitors/{monitor_id}/alert_deliveries"
        delivery_docs = await monitor_service.db.list_documents(
        ...
        for doc in delivery_docs:
            for channel, delivery_data in doc.get("channels", {}).items():
        ...
        from consultantos.models.monitoring import NotificationHistoryResponse
...
async def test_notification(
    ...
    """
    Send test notifications to verify channel configuration.

    Tests delivery to all configured channels (or specific channels if requested)
    without creating an actual alert. Useful for validating Slack webhooks,
    email addresses, etc.

    Args:
        monitor_id: Monitor identifier
        request: Test notification request (optional channel selection)
        user_id: Authenticated user ID
        monitor_service: Intelligence monitor service

    Returns:
        Test delivery results by channel
    ...
    try:
        ...
        channels_to_test = (
            ...
            if request.channels
            else [c.value for c in monitor.config.notification_channels]
        )
        ...
        results = await alerting_service.test_notification(
            user_id=user_id,
            notification_channels=channels_to_test,
        ...
        from consultantos.models.monitoring import TestNotificationResponse
