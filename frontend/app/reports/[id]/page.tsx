...
export default function ReportDetailPage({
  ...
  const loadReport = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await api.analysis.getReport(params.id);
      
      // Transform API response to match ReportData interface
      // The API returns framework_analysis, but frontend expects analysis
      const transformedData: ReportData = {
        id: data.report_id || data.id || params.id,
        company: data.company || '',
        industry: data.industry || '',
        created_at: data.created_at || new Date().toISOString(),
        frameworks: data.frameworks || [],
        depth: 'standard' as const, // Default if not provided
        confidence_score: data.confidence_score || 0,
        status: data.status || 'completed',
        pdf_url: data.report_url || data.pdf_url,
        // Transform framework_analysis to analysis format
        analysis: data.framework_analysis ? {
          porter: data.framework_analysis.porters_five_forces ? {
            competitive_rivalry: data.framework_analysis.porters_five_forces.competitive_rivalry || '',
            supplier_power: data.framework_analysis.porters_five_forces.supplier_power || '',
            buyer_power: data.framework_analysis.porters_five_forces.buyer_power || '',
            threat_of_substitution: data.framework_analysis.porters_five_forces.threat_of_substitution || '',
            threat_of_new_entry: data.framework_analysis.porters_five_forces.threat_of_new_entry || '',
            overall_assessment: data.framework_analysis.porters_five_forces.overall_assessment || '',
          } : undefined,
          swot: data.framework_analysis.swot_analysis ? {
            strengths: data.framework_analysis.swot_analysis.strengths || [],
            weaknesses: data.framework_analysis.swot_analysis.weaknesses || [],
            opportunities: data.framework_analysis.swot_analysis.opportunities || [],
            threats: data.framework_analysis.swot_analysis.threats || [],
          } : undefined,
          pestel: data.framework_analysis.pestel_analysis ? {
            political: data.framework_analysis.pestel_analysis.political || '',
            economic: data.framework_analysis.pestel_analysis.economic || '',
            social: data.framework_analysis.pestel_analysis.social || '',
            technological: data.framework_analysis.pestel_analysis.technological || '',
            environmental: data.framework_analysis.pestel_analysis.environmental || '',
            legal: data.framework_analysis.pestel_analysis.legal || '',
          } : undefined,
          blue_ocean: data.framework_analysis.blue_ocean_strategy ? {
            eliminate: data.framework_analysis.blue_ocean_strategy.eliminate || [],
            reduce: data.framework_analysis.blue_ocean_strategy.reduce || [],
            raise: data.framework_analysis.blue_ocean_strategy.raise || [],
            create: data.framework_analysis.blue_ocean_strategy.create || [],
          } : undefined,
        } : undefined,
      };
  ...
  const loadComments = async () => {
    try {
      const data = await api.comments.list(params.id);
  ...
  const loadVersions = async () => {
    try {
      const data = await api.versions.list(params.id);
  ...
  const handleDeleteReport = async () => {
    ...
    try {
      setIsDeleting(true);
      await api.analysis.deleteReport(params.id);
      router.push("/");
    } catch (err: any) {
  ...
  const handleAddComment = async (text: string) => {
    try {
      await api.comments.create(params.id, text);
      await loadComments();
    } catch (err: any) {
